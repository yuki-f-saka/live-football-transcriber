...
- **用語補正は誤認識された語ではなく文脈側にアンカーする**（#32）。`max_speech` の切断で語尾の子音が落ちると
  `"card"` が `"car"`（普通の英単語）になる。そのため `_CORRECTIONS_EN` は `"(yellow|red) car/cart"` にのみ一致させ、
  `"car"` 単独には一致させない（`"upside position"` も同様で `"upside"` 単独は対象外）。補正は
  `highlights.handle()` の前に走るので、1 つ直すとハイライトのマーカーも復活する。なお同義語の言い換えは補正ではない:
  `"extra time"`（ノックアウトの延長 30 分）や `"added time"` は正しい英語表現なのでそのまま残す。
- **短いチャンクへの `initial_prompt`**: 短いチャンク（1〜2 秒）に長いプロンプトを与えるとプロンプト反復が増えることがある。`FOOTBALL_TERMS_*` は短く保つこと。`--no-vocab` で無効化可能。
